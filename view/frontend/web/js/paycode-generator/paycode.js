define([], function() {
	'use strict';
	return {
		drawPayCode: function(qrcode, canvas, margin, logo, consumer = false) {
			...
			// top left eye
			this.drawEye(ctx, width, margin, cellRadius, 0, 0, offset);
			// top right eye
			this.drawEye(ctx, width, margin, cellRadius, 2*(cellCount - 7)*cellRadius, 0, offset);
			// bottom left eye
			this.drawEye(ctx, width, margin, cellRadius, 0, 2*(cellCount - 7)*cellRadius, offset);
			...
			this.drawIcon(ctx, iconWidth, width, margin, consumer);

			// business logo in the middle
			this.drawLogo(ctx, logo, logoWidth, width, margin, consumer);
		...
		drawEye: function(ctx, width, margin, cellRadius, xOffset, yOffset, offset) {
			// outer edge
			ctx.beginPath();
		...
		drawIcon : function(ctx, iconWidth, width, margin, consumer) {
			var icon = new Image();
		...
		drawLogo: function(ctx, img, logoWidth, width, margin, consumer) {
			ctx.save();
			ctx.beginPath();
